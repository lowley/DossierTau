...
context(BoxScope)
@Composable
fun BottomSheetHeader(modifier:Modifier,item:TauItem?){Column(modifier.wrapContentHeight()){Row(Modifier.fillMaxWidth()){AsyncImage(model=R.drawable.title3,contentDescription="Icone du titre",modifier=Modifier.size(24.dp).align(Alignment.CenterVertically));Text(text=item?.name?.value?:"Aucun titre pour cet élément",modifier=Modifier.padding(start=10.dp).align(Alignment.CenterVertically),style=MaterialTheme.typography.titleMedium,fontWeight=FontWeight.SemiBold,color=Color.DarkGray)};Row(Modifier.fillMaxWidth().padding(top=5.dp)){val extension=if(item?.isFile()==true)item.name.value.substringAfterLast(".").uppercase() else null;if(extension!=null){AsyncImage(model=R.drawable.extensions,contentDescription="Icone du titre",modifier=Modifier.size(24.dp));Text(text=extension,modifier=Modifier.padding(start=10.dp),style=MaterialTheme.typography.titleMedium,color=Color.DarkGray);Text(text="·",modifier=Modifier.padding(start=10.dp,end=10.dp),style=MaterialTheme.typography.titleMedium,color=Color.DarkGray)};val modificationDate=item?.modificationDate?.toddMMyyyyHHmmss();if(modificationDate!=null){AsyncImage(model=R.drawable.calendrier,contentDescription="Calendrier",modifier=Modifier.size(20.dp).align(Alignment.CenterVertically));Text(text=modificationDate,modifier=Modifier.padding(start=10.dp).align(Alignment.CenterVertically),style=MaterialTheme.typography.titleMedium,color=Color.DarkGray)}}}}
