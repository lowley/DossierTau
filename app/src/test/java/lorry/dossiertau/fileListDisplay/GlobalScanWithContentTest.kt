...
//                feature { f((it as ContentItem)::id) }.toEqual(folderToEmit.path)
                feature { f((it as ContentItem)::name) }.toEqual(element.name)
                feature { f((it as ContentItem)::picture) }.toEqual(element.picture?.bitmap?.toByteArray())
                feature { f((it as ContentItem)::memo) }.toEqual(element.memo)
                feature { f((it as ContentItem)::fileId) }.toEqual(element.fileId)
                feature { f((it as ContentItem)::modificationDate) }.toEqual(Instant.ofEpochMilli(element.lastModified))
            }
